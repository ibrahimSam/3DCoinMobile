...
(function() {
  ...
  /* public vars */
  coinjs.pub = 0x17; // 23
  coinjs.priv = 0x82; // 130
  coinjs.multisig = 0x08; // 8
  ...
  /* other vars */
  coinjs.developer = "AZK77QBu3GoUsJ833ZkSyFPLw1hv71PCKc"; //3DCoin
  ...
  coinjs.bech32redeemscript = function(address) {
    console.log("bech32redeemscript    "+address );
    var r = false;
    var decode = coinjs.bech32_decode(address);
    if (decode) {
      decode.data.shift();
      return Crypto.util.bytesToHex(
        coinjs.bech32_convert(decode.data, 5, 8, true)
      );
    }
    return r;
  };
  ...
  coinjs.addressBalance = function(address, callback) {
    ...
    coinjs.ajax(
      "https://3dcstats.net/insight-api/addr/" + address + "/balance",
      callback,
      "GET"
    );
  };
  ...
  coinjs.bech32_decode = function(bechString) {
    console.log("function(bechString)  --->  " + bechString);
    var p;
    var has_lower = false;
    var has_upper = false;
  ...
  listUnspent_3dcstatsnet = function(redeem) {

    coinsjs.ajax({
      type: "GET",
      url: "https://3dcstats.net/insight-api/addr/" + redeem.addr + "/utxo",
      dataType: "json",
      error: function(data) {
  ...
  coinjs.transaction = function() {
    ...
    r.addoutput = function(address, value) {
      var o = {};
      o.value = new BigInteger("" + Math.round(value * 1 * 10000000000), 10);
      var s = coinjs.script();
        ...
        for (i = 1; i <= unspent.childElementCount; i++) {
          ...
          var txhash =
            u
              ...
              .reverse()
              .join("") + "";
          var n = u.getElementsByTagName("tx_output_n")[0].childNodes[0]
            .nodeValue;
          var scr =
            script ||
    ...
    r.broadcast = function(callback, txhex) {
      ...
      coinjs.ajax(
        coinjs.host +
          ...
          "&key=" +
          coinjs.key +
          "&setmodule=bitcoin&request=sendrawtransaction&rawtx=" +
          tx +
          "&r=" +
          Math.random(),
        callback,
